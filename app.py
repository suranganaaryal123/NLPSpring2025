...
def calculate_match_score(resume_text, job_description):
    if not job_description:
        return None  # Skip if no JD provided
    ...
    if st.button("Analyze Resume"):
        with st.spinner("Analyzing resume..."):
            try:
                ...
                # Show Gemini's feedback
                st.subheader("AI-Powered Resume Analysis")
                st.write(analysis)

                # Show match score using cosine similarity
                if job_description:
                    ...
                    if match_score is not None:
                        ...
                        if match_score >= 75:
                            st.success("Great match! You’re well aligned for this job.")
                        elif match_score >= 50:
                            st.warning("Moderate match. Some improvements can help.")
                        else:
                            st.error("Low match. Consider tailoring your resume more closely to the job.")
            except Exception as e:
